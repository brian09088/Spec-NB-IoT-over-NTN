...
%% 子訊框消耗個數 NTN_subframes: Calculate subframes consumed

NTN_subframes_UL = (nBits + nCRC) / (nRU * nSymbols * m * nRep);
% nBits + nCRC = 208 + 24 = 232
% nRU * nSymbols * m * nRep = 1*160*2*1 = 320
fprintf("UpLink NTN_subframes consumed: %.2f\n", NTN_subframes_UL);

m = 2;                % Bits per symbol for QPSK
NTN_subframes_DL = (nBits + nCRC) / (nSF * nSymbols * m * nRep);
% nBits + nCRC = 208 + 24 = 232
% nSF * nSymbols * m * nRep = 8*160*2*1 = 2560
fprintf("DownLink NTN_subframes consumed: %.2f\n", NTN_subframes_DL);

%% 資源利用率 NTN_utilization
% nRU : 上行鏈路資源單位數 number of uplink resource unit
% nDSC : 資料子載波數量/上行鏈路 number of subcarriers
BW = bandwidth_kHz;           % bandwidth = 180kHz
UL_utilization = (nRU * nDSC) / BW; 
% NTN_utilization = (1 * 72) / 180
fprintf("NTN_UL_utilization ratio: %.2f\n", UL_utilization);

% nSF : 下行子訊框數 number of uplink subframes
% nDSC : 資料子載波數量/下行鏈路 number of subcarriers
BW = bandwidth_kHz;           % bandwidth = 180kHz
DL_utilization = (nSF * nDSC) / BW; 
% NTN_utilization = (8 * 72) / 180
fprintf("NTN_DL_utilization ratio: %.2f\n", DL_utilization);

%% 流通率 NTN_Throughput
% NB-IoT NTN NPDSCH Throughput
% https://www.mathworks.com/help/satcom/ug/nb-iot-ntn-npdsch-throughput.html
Rs = 1/td;                          % 符號速率 (Symbol rate, Rs)
M = 4;                              % QPSK, 調變方案 (Modulation scheme, M)
eff = nBits / (nBits + nCRC);       % 編碼效率 (Coding efficiency, eff)
% eff = 208 / 208+24 = 0.8966 
% 編碼效率反映了在編碼過程中，實際有用的信息bit相對於總傳輸bit的比例
q_factor = 1;                 % Channel quality factor = 1
NTN_throughput = Rs * log2(M) * eff * q_factor;
max_throughput = Rs * log2(M) * q_factor;
fprintf("NTN_throughput ratio: %.2f\n", NTN_throughput / max_throughput);
...
% Received power (dBm) = modulated output power (dBm) + high power amplifier gain (dB) + antenna gain (dB) - FSPL(dB),
